...
public class Timeline_Activity extends Activity_Superclass {
    ...
    Timeline_Activity(WifiDataBuffer buf, Activity_Superclass cali_tables, final int device_id, final int attenuator, final char measurement_type, int freq){
    ...
    public void startMeasurement (int device_id, int attenuator, char measurement_type) {
        ...
        while(true) {
            while (!buffer.isDataWaiting_FromESP()) {
                try {
                    wait(10);
                    Build_Packet_Test testpacket = new Build_Packet_Test(95); //packet from exposi
